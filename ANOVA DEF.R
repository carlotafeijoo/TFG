...
# install.packages(c("afex", "emmeans", "tidyverse"))
...
# Load the data
data <- read_delim("C:/Users/carlo/OneDrive/Documentos/Carlota/CEU/TFG/Tabla todos medias ROM bien.csv", 
...
# post-hoc comparisons and tam of effect
# when in NOVA there is sig dif when using comparative of exo:weight 
#weight:
emm_exo <- emmeans(anova_rom, ~ Exo | Weight)
pairs_exo <- pairs(emm_exo, adjust = "holm") #Holm/Bonferroni
eff_size(pairs_exo, sigma = sigma(anova_rom), edf = df.residual(anova_rom)) 
#weight:
emm_weight <- emmeans(anova_rom, ~ Weight | Exo)
pairs_weight <- pairs(emm_weight, adjust = "holm") #Holm/Bonferroni
eff_size(pairs_weight, sigma = sigma(anova_rom), edf = df.residual(anova_rom))
...
p_cc_flex_accel <- ggplot(data, aes(x = Condition, y = CC_Flex_Accel, fill = Condition)) +
  ...
  geom_signif(
    comparisons = list(
      c("Exo - Weight", "Exo - No Weight"),
      c("Exo - Weight", "No Exo - Weight"),
      c("Exo - Weight", "No Exo - No Weight")
    ),
    annotations = c("*", "**", "***"),
